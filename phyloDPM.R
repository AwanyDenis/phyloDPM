...
#' The Bayesian model produces posterior estimates of OTU effects, and other results for post-hoc analyses. 
#'
#' @param Y A binary response vector of the phenotype.
#' @param Z A sample by taxa matrix or dataframe of microbiome abundance.
#' @param mTree A phylogenetic tree in the form of a matrix or a phylo object.
#' @param Nsim Integer; number of Markov Chain Monte carlo (MCMC) simulations. Default: 25000.
#' @param thin Integer; number of posterior draws to be used for thining.
#' @param burnin Integer; number of burn in samples. Default taken to be 10% of Nsim.
#' @param post.plots TRUE or FALSE; whether or not to display plots of posterior samples. Default TRUE
#' @param mu Tuning parameter for phylogenetic and RBF kernels. Default 0.5 to place equal weighting to information to phylogenetic tree and RBF kernels.
#' @param evol.rate Numeric; paremeter for trait evolution model. Default 10^3.
#' @param alpha.par First hyper-parameter for Gamma prior of precision parameter for the DP. Default NULL
#' @param tau2.par Variance parameter on the base parameter of the DP. Default sample from gamma distribution with parameters (20,20).
#' @param sig2.bet.par Variance parameter on beta. Default sample from Inverse Gamme IG (20,15).
#' @param rbf.par Kernel coefficient for the RBF kernel. Default 0.001
#' @param saveSim TRUE or FALSE; whether or not to save the Monte Carlo simulation results. 
#' @param otu.corr TRUE or FALSE; whether or not to save the Monte Carlo simulation results. 
...
phyloDPM.main <- function(Y, #case-control phenotype [vector]
                          ...
                          Nsim=25000, #number of MCMC iterations [integer]
                          thin, # thinning value for posterior samples [integer]
                          burnin=NULL, #number of burn-in samples [integer]
                          post.plots=TRUE, # whether or not to display plots of posterior samples
                          ...
                          otu.corr, #logical; whether or not to consider correlation among OTUs.
                          ...){
...
thin.samples <- function (x, thin) {
 ...
 hpd_interval <- function(obj, prob = 0.95, ...) {
  ...
  sig.sq.beta    <- rinvgamma(1,shape=a.beta,scale=b.beta)
  if(!is.finite(sig.sq.beta)) {sig.sq.beta <- 1.0e+50} # If Infinity, use 1.0e+50 to represent the large enough infinite value!!
  sig.beta    <- sqrt(sig.sq.beta)
  ...
  if(otu.corr==FALSE) joint.cov=diag(num.otu)

  ## Add inrtercept to the data
  Z      <- cbind(1,Z) #

  ## With intercept, add data of zero correlations for it.
  int.mat <- matrix(0, nrow=ncol(Z),ncol=ncol(Z)) ; int.mat[1,1] <- 1
  int.mat[-1,-1] <- joint.cov
  joint.cov <- int.mat
  ...
  tau.sq   <- c(tau.sq, rep(NA,Nsim))
  sig.sq.beta <- c(sig.sq.beta, rep(NA,Nsim))
  
  #name the matrices and vectors [variables]:
  colnames(Beta)  <- c("intercept",colnames(otu.mat))
  names(tau.sq)   <- "tau.sq"
  names(sig.sq.beta) <- "sig.sq.beta"
  ...
  like.K <- 0 #
  for (i in 1:n){
    like.K <- like.K + (dbinom(Y[i], size=1, prob=pr[i], log=TRUE)) + (dnorm(psi[i], mean=0, sd=tau,log=TRUE))
    like.K <- exp(like.K + sum(lgamma(A.K)))  #
  }

  # For the prior of m, we use Gamma(a,b)
  m = initial.m = n/log(n) # precision parameter of the DP part. Initialize by n/log(n)
  m = c(m, rep(NA,Nsim))
  names(m) = "m"
  ...
  for (iter in 2:(Nsim+1)){  # simulate samples (particles).
    ...
    like.K.can        <- 0
    for(i in 1:n){
      like.K.can <- like.K.can + (dbinom(Y[i], size=1, prob=pr[i], log=TRUE)) + (dnorm(psi[i], mean=0, sd=tau, log=TRUE)) #
    }
    ...
    rho <- p.ratio * f.ratio * mult.ratio # Acceptance probability/ratio.
    if (rho>runif(1)) {
        A.n        <- A.n.can
        A.K        <- A.K.can
    ...
    for (j in 1:dim(Z)[2]){
        ...
        Mu[j] <- rnorm(n=1, mean=1/(1/sig.beta^2 + as.numeric(solve(t(joint.cov[,j])%*%joint.cov[,j]))) * 
        	((1/sig.beta^2)*Beta[iter-1,j] + as.numeric(solve(t(joint.cov[,j])%*%joint.cov[,j]))%*%Gama[j]), 
        	sd=1/sig.beta^2 + as.numeric(solve(t(joint.cov[,j])%*%joint.cov[,j])) )          
        ...
        a1 <- pnorm(beta.left, Mu[j], sig.beta)   #
        a2 <- pnorm(beta.right, Mu[j], sig.beta)
        pa <- a1 + unif.random1*(a2-a1)
        if(pa > 0.99999) {pa=0.99999} # Avoid numerical issues, since qnorm(1,mean,sd)==Inf
        current.beta.j <- qnorm(pa, Mu[j], sig.beta)
        #Or: 
        #current.beta.j <- rtnorm(n=1, mean = Mu[j], sd = sig.beta, lower = beta.left, upper = beta.right)
    ...
    for (k in 1:K){
      # Lower limit for eta
       ...
       eta.left  = max(eta.lim1) #
       if(!is.finite(eta.left)) eta.left=-Inf 
       eta.right = min(eta.lim2) # 
       if(!is.finite(eta.right)) eta.right=Inf 

       #simulating from truncated normal distribution.
       #Either:
       unif.random2 <- runif(1,0,1)
       b1 <- pnorm(eta.left, 0, tau)
       b2 <- pnorm(eta.right, 0, tau)
       pb <- b1 + unif.random2*(b2-b1)
       if(pb > 0.99999) {pb=0.99999} #Avoid numerical issues, since qnorm(1,mean,sd)==Inf
       current.eta.k <- qnorm(pb, 0,  tau)
       #Or:
       #current.eta.k <- rtnorm(n=1, mean=0, sd=tau, lower=eta.left, upper=eta.right)
       eta[k] <- current.eta.k 
    }
    ...
    ## =====================
    # Perform conditional update on tau2, sigma2.beta.
    # simulate tau2
    tau2 <- rinvgamma(1,shape=(0.5*K)+a.tau, scale=(0.5*sum(eta^2))+b.tau )
    
    if (is.finite(tau2)){
      tau <- sqrt(tau2) 
    }else{  #since tau2 can go to infinity.
    	tau2 <- 1e50 # if value goes infinite,  1e50 can represent infinity??
      tau <- sqrt(tau2) 
    } #
    tau.sq[iter] <- tau2 # save tau2
    ...
    if(is.finite(sig2.beta)){ 
      sig.beta <- sqrt(sig2.beta)
    }else{ #since sig2.beta can go to infinity.
      sig2.beta = 1e50 # If infinite, 1e50 can represent infinity??
      sig.beta  <- sqrt(sig2.beta) #
    } #
    sig.sq.beta[iter] <- sig2.beta

    ## ===================
    # Update concentration paremeter,'m':
    # inputs: [previous (m), sample size (n), cluster size (K), Gamma params (a,b) on m].
   ...
   if (saveSim){
      ...
      out.df <- cbind(Beta, sig.sq.beta, m, tau.sq)
      column.names <- c(paste0("OTU", 1:num.otu), "sigma2.beta", "m", "tau.sq")
  ...
  tau2.Burn         <- tau.sq[(burnin+1):Nsim]
  sig2.beta.Burn    <- sig.sq.beta[(burnin+1):Nsim]
  m.Burn            <- m[(burnin+1):Nsim]

  # thinning
  Beta.BurnThin     <- thin.samples(as.matrix(Beta.Burn), thin) 
  tau2.BurnThin     <- thin.samples(as.vector(tau2.Burn),thin)
  sig2.beta.BurnThin<- thin.samples(as.vector(sig2.beta.Burn),thin)
...
#              Posterior summary
# =================================================
posterior.summary <- function(object, alpha=0.5, ...){
#
...
#
  ...
  post.mean.beta=round(apply(X,2,mean), digits=3)
  ...
  num_se = round(num_se,3); rel_eff= format(round(rel_eff,3), nsmall=3)

  sds.beta= round(apply(X,2,sd),3) # posterior stand devs.
  lo = post.mean.beta - qnorm(1-alpha/2) * num_se
  up = post.mean.beta + qnorm(1-alpha/2) * num_se   
  CI = format(round(cbind(lo, up),3), nsmall=3)

  mat = cbind(format(post.mean.beta,nsmall=3), format(num_se,nsmall=3), CI)
  
  #names of variables
  rownames(mat)=c("(intercept)",colnames(otu.mat),"m")
  colnames(mat)[1]="Coef" #Estimated coefficients
  #colnames(mat)[2]="std.dev" #posterior stan devs
  colnames(mat)[2]="SE" # numerical standard errors
  #colnames(mat)[4]="rel.eff" # relative efficiency
  colnames(mat)[3]="95% CI(lo)" # Credible Interval (lower)
  colnames(mat)[4]="95% CI(up)" # (upper)
  
  # format the output results
  mat=format(mat, justify = "centre", digits=3, scientific=FALSE) 

  cat("\nEstimates of the parameters:\n")
  print((mat),digits=3, quote=FALSE)
...
graphs.function <- function(X, tvalues,...){
  ...
  if(post.plots){
     ...
     for(index in 1:nx){
        plot(as.vector(X[,index]),xlab="Iteration",ylab="",main=paste0("Trace plot for ",colnames(X)[index]),type="l",col="red") #traceplot
        plot(density(X[,index]), xlab="Value",ylab="Density",main=paste0("Density plot for ",colnames(X)[index]), type="l",col="blue") #density plot
        cumuplot(mcmc(X[,index]),probs=c(0.025,0.5,0.975),ylab="",xlab="Iteration",main=paste0("Cumulative quantile plot for ",colnames(X)[index]),
        ...
        if(var(X[,index])>1.0e-20) {acf(as.vector(X[,index]),xlab="Lag",ylab="Autocorrelation",main=paste0("ACF plot for ",colnames(X)[index]))} #ACF plot
...
post.est <- posterior.summary(cbind(Beta.BurnThin,m.BurnThin))

# # Make posterior plots using ALL the simulated data (no thin and no burn-in)
if(post.plots){
  graphs.function(cbind(Beta[,1:5])) #posterior plots for OTU effects..
}
...
#================================================
phyloDPM <- function(pheno, otu.data, mTree, Nsim, thin=1, burnin=NULL, post.plots=FALSE, mu=0.5 
                     alpha.par=NULL, tau2.par=c(20,20),sig2.bet.par=c(20,15), rbf.par=0.001, saveSim=FALSE,evol.rate=10^3,otu.corr=TRUE,...) {
  ...
  est <- phyloDPM.main(Y = pheno, Z = otu.data, mTree = mTree, tau2.par=tau2.par, sig2.bet.par=sig2.bet.par,
  			...
  			post.plots=post.plots, mu=mu, rbf.par=rbf.par,saveSim=saveSim,evol.rate=evol.rate,otu.corr=otu.corr,...)
  #class(est) <- "phyloDPM"
  est	
}
