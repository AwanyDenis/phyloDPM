...
options(max.print=999999) # print full results to screen



# Now run the model.
source('./phyloDPM.R')
out1 <- phyloDPM(y,otu.abun,mTree=otu.tree,mu=0,alpha.par=c(a1,b1),Nsim=50000,post.plots=TRUE)
...
## ================================================================================
# HEATMAP FOR THE SIMULATED OTUs
## ================================================================================
# Heatmap of simulated OTUs
# Plotting heatmap of OTU counts.
# We will draw heatmap for only most abundant OTUs (those with counts)
#otu4.plot = OTU.counts.simulated[,colSums(OTU.counts.simulated) !=0 ]
# The plot of simulated OTU counts

png(filename="heatmap_simOTU_counts.png", width=1000, height=800)
row.names(otu.counts) <- paste0("S",1:nrow(otu.counts))
plot_heatmap(otu_table(otu.counts, taxa_are_rows = FALSE),
             sample.label = NULL, taxa.label = NULL,
             na.value = "black", low="#000033", high="#66CCFF",
             max.label = 250, title = NULL, sample.order = NULL, taxa.order = NULL,
             first.sample = NULL, first.taxa = NULL)
dev.off()

# The plot of real OTU counts  (from which parameters for simulating OTUs were 
#determined)

png(filename="heatmap_realOTU_counts.png", width=1000, height=800)
plot_heatmap(otu_table(throat.otu.tab, taxa_are_rows = FALSE),
             sample.label = NULL, taxa.label = NULL,
             na.value = "black", low="#000033", high="#66CCFF",
             max.label = 250, title = NULL, sample.order = NULL, taxa.order = NULL,
             first.sample = NULL, first.taxa = NULL)
